...
### 📋 JD Analysis
Upload a job description (PDF, DOCX, or TXT) or **generate one from scratch** — the JD Analysis tab has three sub-tabs:

- **Generate JD** — describe the person you want to hire (skills, experience, seniority, industry) and the AI creates a professional, well-structured job description. The generated JD automatically becomes the active JD for analysis and evaluation. You can then **refine it via chat** — request changes, adjust tone, add or remove sections, and the AI rewrites the full JD with your modifications applied.
- **Analysis** — get a structured, easy-to-read breakdown of the active JD:
  - Role summary
...
- **Chat with JD** — ask follow-up questions about the active job description, clarify requirements, brainstorm sourcing strategy, or get advice on screening — in a normal chat interface with streaming responses. Download the full chat log as Markdown.
...
### Analyzing or generating a job description
1. **To upload a JD:** Upload it in the **sidebar** (this makes it "active" across every tab).
2. **To generate a JD:** Go to the **JD Analysis > Generate JD** sub-tab, describe your ideal hire, and click **Generate JD**. The result automatically becomes the active JD.
3. Go to the **Analysis** sub-tab and click **Analyze Job Description** to get a structured breakdown.
4. Review the breakdown. Click any LinkedIn X-ray search's **"Search →"** button to open it directly in Google, scoped to LinkedIn profiles.
5. Download the analysis as Markdown or JSON, or use the **Chat with JD** sub-tab to ask follow-up questions.
